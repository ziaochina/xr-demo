(function webpackUniversalModuleDefinition(root, factory) {
	if(typeof exports === 'object' && typeof module === 'object')
		...
		exports["XRAppLoader"] = factory(require("Immutable"), require("React"), require("ReactRedux"), require("Redux"), require("ReactDom"));
	else
		root["XRAppLoader"] = factory(root["Immutable"], root["React"], root["ReactRedux"], root["Redux"], root["ReactDom"]);
})(this, function(__WEBPACK_EXTERNAL_MODULE_20__, __WEBPACK_EXTERNAL_MODULE_35__, __WEBPACK_EXTERNAL_MODULE_36__, __WEBPACK_EXTERNAL_MODULE_37__, __WEBPACK_EXTERNAL_MODULE_120__) {
...
var appFactory = function () {
    ...
    (0, _createClass3.default)(appFactory, [{
        ...
        value: function registerComponents(apps) {
...
var _parseName = __webpack_require__(16);
...
var _createReduxConnector = __webpack_require__(41);
...
exports.default = (0, _reactRedux.connect)(function (state, props) {
	var parsedName = (0, _parseName2.default)(props.name),
		...
		return function (next) {
			return function (action) {
				...
				if (typeof action === 'function') {
					...
					var reduce = function reduce(type) {
						...
						dispatch({
							...
							payload: {
								...
								reducer: reducer,
								payload: args,
								injectFunsForReducer: injectFunsForReducer
							}
						});
					};
					...
					var realAction = actionCreator.apply(undefined, (0, _toConsumableArray3.default)(args));
					if (typeof realAction === 'function') {
						realAction((0, _extends3.default)({
							store: store,
							reduce: reduce,
					...
					var fullName = action.payload.fullName,
					    parsedName = (0, _parseName2.default)(fullName);

					_appFactory2.default.getApp(parsedName.name).load(function (component, action, reducer) {
						return next({
							type: '@@loadAppReal',
							payload: {
								fullName: fullName,
...
var _wrapMapStateToProps = __webpack_require__(64);
...
function loadApp(state, _ref2) {
    ...
    if (!state.has(name)) {
        state = state.set(name, (0, _immutable.Map)());
        state = state.setIn([name, '@@require'], (0, _immutable.Map)({
            component: component,
            action: action,
            reducer: reducer
        }));
    ...
    if (!state.getIn([name, '@@require', fullName])) {
        var container = (0, _createReduxConnector2.default)(component, (0, _wrapMapStateToProps2.default)(fullName), (0, _wrapMapDispatchToProps2.default)(fullName, action, reducer), null, {
            withRef: true,
            pure: true
        });
    ...
    if (query !== '' && !state.get(name).has(query)) {
        state = state.update(name, function (x) {
...
function clearAppState(state, _ref3) {
    ...
    if (query !== '' && state.get(name).has(query)) {
        ...
        var ks = [];
        state.get(name).mapKeys(function (k) {
            if (k != '@@require' && k.indexOf('=') == -1) ks.push(k);
            return k;
        });

        ks.forEach(function (k) {
            if (k) state = state.update(name, function (x) {
                return x.remove(k);
            });
        });
    }
...
function reduce(state, _ref4) {
    ...
    var oldState = query !== '' ? state.getIn([name, query]) : state.get(name);
...
function wrapMapDispatchToProps(fullName, actionCreators, reducer) {
	...
	for (var i = 0; i < keys.length; i++) {
		var key = keys[i];
		var wrapActionCreator = wrapAction(actionCreators[key], reducer, parsedName.name, parsedName.query);
...
var _appMiddleware = __webpack_require__(40);
...
var _start = __webpack_require__(63);
...
function config(option) {
	...
	_appFactory2.default.registerApps(apps);
}
module.exports = exports['default'];
...
function start(targetDomId, middlewares, startAppName) {
	var mw = [(0, _appMiddleware2.default)()];
	if (middlewares) mw = mw.concat(middlewares);
	...
	(0, _reactDom.render)(_react2.default.createElement(
		...
		{ store: store },
		_react2.default.createElement(_appLoader2.default, { name: startAppName })
